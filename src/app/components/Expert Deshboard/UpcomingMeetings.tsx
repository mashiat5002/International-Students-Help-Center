        ...
        {details.map((item,k) => (
          <li key={k} className="flex flex-col md:flex-row md:items-center justify-between bg-white/5 rounded-xl p-4 border border-white/10">
            ...
            <div className="mt-4 md:mt-0 md:ml-6 flex flex-col gap-2 items-end">
              ...
              />:<p className='text-white'>{`Scheduled on ${new Date(item.Scheduled_time).toLocaleString()}`}</p>}
              <div className='w-full h-fit flex '>

              <button
                className={` mt-2 px-5 py-2 hover:bg-red-400 text-white rounded-full font-medium shadow  transition`}
