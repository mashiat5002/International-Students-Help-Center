...
export default function ExpertRegisterCard({ isOpen, onClose, onShowLogin, initialEmail }: RegisterCardProps) {
  ...
  const handleResendOtp = async () => {
    ...
    setIsResending(true);
    try {
      const response = await call_expert_update_varification_key_db(formData.email);
  ...
  const handleSubmit = async (e: React.FormEvent ) => {
    ...
    try {
      const response = await call_register_expert(formData.fullName,formData.email,formData.password);
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await call_activate_expert_db(formData.email, otp);
      ...
      <div className="bg-white/10 backdrop-blur-md p-8 rounded-xl shadow-2xl w-full max-w-md mx-4 border border-white/20"
        ...
        <div className="text-center mb-2">
          <h2 className="text-3xl font-bold text-white mb-2">
            {showOtpForm ? 'Verify Your Email' : 'Register as Consultant'}
          </h2>
        ...
        {!showOtpForm ? (
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
              ...
              >
                {isRegistering ? 'Creating Account...' : `Create Account as Expert`}
              </button>
